...
class _ConvBatchNorm2D(convolutional.Conv2D):
  ...
  def call(self, inputs, training=None):
    ...
    if self.is_quantized:
      folded_conv_kernel = _apply_weight_quantizer(self, training,
                                                   folded_conv_kernel)
  ...
  def get_config(self):
    ...
    # Branching is required since self.post_activation can be either
    # the activation or a QuantizeAwareActivation depending on whether
    # self.build() has been called.
    if isinstance(self.post_activation,
                  quantize_aware_activation.QuantizeAwareActivation):
      post_activation_object = self.post_activation.activation
    else:
      post_activation_object = self.post_activation

    config = {
        'is_quantized':
            self.is_quantized,
        'post_activation':
            keras.utils.serialize_keras_object(post_activation_object),
    }
...
class _DepthwiseConvBatchNorm2D(convolutional.DepthwiseConv2D):
