...
def _build_for_quantization(self):
  ...
  self._weight_min_var = self.add_variable(
      'weight_min', initializer=initializers.Constant(-6.0), trainable=False) # pylint: disable=protected-access
...
def _from_config(cls, config):
  ...
  # comment in __init__.
  config.pop('use_bias')
  config['post_activation'] = activations.deserialize(config['post_activation'])
